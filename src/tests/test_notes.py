...
def test_create_note_invalid_json(test_app):
    response = test_app.post("/notes/", content=json.dumps({"title": "foo"}))
...
def test_read_all_notes(test_app, monkeypatch):
    test_data = [
        {"title": "foo", "description": "bar", "id": 1},
        {"title": "bar", "description": "foo", "id": 2},
    ]
...
def test_update_note(test_app, monkeypatch):
    test_update_data = {"title": "foo", "description": "bar", "id": 1}
